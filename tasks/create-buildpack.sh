...
PHP_5_BINARY_SHA256=$(sha256sum /php-binaries/php-$PHP_5_VERSION-linux-x64.tgz | awk '{print $1}')
...
PHP_7_BINARY_SHA256=$(sha256sum /php-binaries/php-$PHP_7_VERSION-linux-x64.tgz | awk '{print $1}')
sed -i s/'##php-7-version##'/$PHP_7_VERSION/g manifest.yml
sed -i s/'##php-7-binary-location##'/$PHP_7_BINARY_LOCATION/g manifest.yml
